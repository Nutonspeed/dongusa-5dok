...
export class EnhancedPaymentService {
  ...
  private async processCreditCard(transactionId: string, amount: number, paymentData: any) {
    if (!this.stripeGateway) {
      return this.processMockCreditCard(transactionId, amount, paymentData)
    }
  ...
  private async processMockCreditCard(transactionId: string, amount: number, paymentData: any) {
    // Mock implementation for development
    const mockPaymentIntentId = `pi_mock_${Date.now()}`

    await this.updateTransactionStatus(transactionId, "pending", mockPaymentIntentId)
