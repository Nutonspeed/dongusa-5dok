...
// Load local .env files when running this script directly (helps local dev and CI helpers)
...
try {
  ...
  // ignore dotenv errors — health-check will report missing envs
}
...
class WebsiteHealthChecker {
    ...
    for (const file of criticalFiles) {
      try {
          ...
          for (const importLine of imports) {
            ...
            if (importLine.includes("../") && importLine.includes("@/")) {
              this.addResult(
                "Imports",
                "warning",
                `Mixed import styles in ${file}: ${importLine}`,
              );
              hasIssues = true;
            }
          }
        ...
        this.addResult(
          "Imports",
          "fail",
          `Error checking imports in ${file}: ${error}`,
        );
      }
    }
      ...
      if (reactVersion && nextVersion) {
        if (reactVersion.includes("19") && !nextVersion.includes("15")) {
          this.addResult(
            "Dependencies",
            "warning",
            "React 19 with Next.js < 15 may cause compatibility issues",
          );
        } else {
          this.addResult(
            "Dependencies",
            "pass",
            "React and Next.js versions are compatible",
          );
        }
      }
  ...
  private generateReport(): void {
    ...
    if (summary.fail > 0) {
      this.log(
        "❌ Health check completed with failures. Please address the issues above.",
        "error",
      );
      process.exit(1);
    } else if (summary.warning > 0) {
      this.log(
        "⚠️  Health check completed with warnings. Consider addressing them.",
        "warning",
      );
    } else {
      this.log(
        "✅ All health checks passed! Your website is ready.",
        "success",
      );
    }
  }
