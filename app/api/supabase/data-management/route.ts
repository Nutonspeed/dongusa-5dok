export const runtime = "nodejs"; // Use Node.js runtime for Supabase data management
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { success: false, error: "Failed to generate data management report" },
      { status: 500 },
    );
  }
}
...
export async function POST() {
  try {
    ...
    return NextResponse.json(
      { success: false, error: "Failed to execute data management strategy" },
      { status: 500 },
    );
  }
}
